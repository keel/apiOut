...
const t2345 = function(phone, callback) {
  ktool.httpGet('http://tools.2345.com/frame/black/mobile/' + phone, (err, re) => {
    ...
    let p1 = htmlStr.indexOf('<strong>' + phone + '</strong>') + 9;
    p1 = htmlStr.indexOf('<strong>', p1) + 8;
    const p2 = htmlStr.indexOf('</strong>', p1);
    const locationStr = htmlStr.substring(p1, p2);
