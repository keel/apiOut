/**
 * 省份按号码查找并合并到表,生成provinceSeq表,使用provinceSeq:seq开头的redis键进行缓存
 */
'use strict';
...
const provinceTable = 'provinceSeq';
...
const push = function(phone, re) {
  ...
  const seq = phone.substring(0, 7);
  const mongoData = { 'seq': seq, 'provinceName': re.province, 'cityName': re.city };
  // 入库操作
  mongo.c(provinceTable).logToDb(mongoData, function(err) {
    if (err) {
      vlog.eo(err, 'add province seq to mongo 存储失败');
    }
  });

  const redisData = { 'seq': seq, 'provinceName': re.province, 'cityName': re.city };

  redis.hmset(provinceRedisPre + seq, redisData, function(err) {
    if (err) {
