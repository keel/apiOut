...
const dianboCostUpdate = (apiName, productObj, userId, provinceName, fee, callback = ktool.defaultCallback(null, __filename)) => {
  ...
  db.c(apiName + '_user').updateOne({ 'userId': userId, 'productKey': productObj.productKey }, { '$inc': { 'dianBoDayCost': feeInt, 'dianBoMonthCost': feeInt } }, option, (err) => {
    if (err) {
    ...
    db.c(riskLimitTable).updateOne(query, { '$inc': { 'dayFeeNum': feeInt, 'monthFeeNum': feeInt } }, (err) => {
      if (err) {
...
const vrDianboUserCheck = function vrDianboUserCheck(apiName, userId, fee, dianBoDayUserLimit, dianBoMonthUserLimit, callback) {
  // console.log(apiName, userId, fee, dianBoDayUserLimit, dianBoMonthUserLimit);
  fee = fee || 0;
  dianBoDayUserLimit = dianBoDayUserLimit || 0;
  dianBoMonthUserLimit = dianBoMonthUserLimit || 0;
  db.c(apiName + '_user').findOne({ 'userId': userId }, (err, userOne) => {
    ...
    if (dianBoDayUserLimit > 0 && userOne.dianBoDayCost + fee > dianBoDayUserLimit) {
      return callback(null, errorCode.err.dianBoDayUserCost);
    }
    if (dianBoMonthUserLimit > 0 && userOne.dianBoMonthCost + fee > dianBoMonthUserLimit) {
...
const vrDianboRiskCheck = (apiName, phone, fee, productKey, callback) => {
  provinceUtil.findProvinceByPhone(phone, (err, provinceName) => {
    ...
    db.c(riskLimitTable).findOne(query, (err, doc) => {
      ...
      vrDianboUserCheck(apiName, phone, parseInt(fee), doc.dianBoDayUserLimit, doc.dianBoMonthUserLimit, (err, vrDianboUserCheckRe) => {
        if (err) {
