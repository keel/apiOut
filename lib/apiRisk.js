...
//点播用户表日月限检查
const vrDianboUserRisk = function vrDianboUserRisk(apiName, userId, fee, dianBoDayUserLimit, dianBoMonthUserLimit, callback) {
  ...
  db.c(apiName + '_user').findOne({ 'userId': userId }, (err, userOne) => {
    ...
    if (dianBoDayUserLimit > 0 && userOne.dianBoDayUserCost + fee > dianBoDayUserLimit) {
      return callback(null, errorCode.err.dianBoDayUserCost);
    }
    if (dianBoMonthUserLimit > 0 && userOne.dianBoMonthUserCost + fee > dianBoMonthUserLimit) {
...
const vrDianboRiskCheck = (apiName, phone, fee, productKey, callback) => {
  provinceUtil.findProvinceByPhone(phone, (err, provinceName) => {
    ...
    db.c(riskLimitTable).findOne(query, (err, doc) => {
      ...
      vrDianboUserRisk(apiName, phone, parseInt(fee), doc.dianBoDayUserLimit, doc.dianBoMonthUserLimit, (err, vrDianboUserRiskRe) => {
        if (err) {
