...
const syncAction = function syncAction(apiName, action, respObj) {
  if (!respObj.sync_obj) {
  ...
  syncFindOrder(apiName, respObj.orderQuery, function(err, orderObj) {
    ...
    //用户关系表更新
    if (respObj.sync_user && respObj.sync_feeId) {
      const userRelation = {
    ...
    if (respObj.sync_type === 0) {
      db.c(apiName + '_order').updateOne({ '_id': orderObj._id }, { '$set': { 'sync_re_code': respObj.sync_re_code, 'sync_re_desc': respObj.sync_re_desc, 'state': (respObj.sync_re_code === '0') ? 4 : 3 } }, { 'multi': false }, function(err) {
        if (err) {
      ...
      if (respObj.sync_re_code + '' === '0') {
        findProduct(orderObj.productKey, (err, productObj) => {
          if (err) {
            return vlog.eo(err, 'sync:getProductInfo');
          }
          //TODO 这里需要从action得到risk对象
          const riskChecker = action.riskChecker;
          riskChecker.usageUpdate(orderObj.productKey, orderObj.provinceName, parseInt(orderObj.fee));
...
const actionMap = {
  verify(apiName, reqObj, productObj, callback) {
    db.c(apiName + '_order').findOne({ 'orderId': reqObj.orderId }, function(err, orderObj) {
      if (err) {
