...
const findProduct = (productKey, callback) => {
  ...
  db.c(productTable).findOne(query, (err, doc) => {
    ...
    // if (doc.dayLimit && doc.dayCost && doc.dayCost >= doc.dayLimit) { //增加日限----在业务侧添加
    //   return callback();
    // }
...
const sendToCp = function(apiName, respObj, orderObj) {
  ...
  findProduct(orderObj.productKey, (err, productObj) => {
    ...
    const postCmd = (productObj.callbackUrl.startsWith('https')) ? ktool.httpsPost : ktool.httpPost;
    postCmd(productObj.callbackUrl, JSON.stringify(sendData), function(err) {
      if (err) {
...
const syncAction = function syncAction(apiName, apiType, action, respObj) {
  ...
  syncFindOrder(apiName, respObj.orderQuery, function(err, orderObj) {
    ...
    if (orderObj) {
      ...
      userRelationUpdate.orderId = orderObj.orderId;
      userRelationUpdate.cpOrder = orderObj.cpOrder;
    }
    ...
    findKeyByProduct(respObj, userRelationUpdate, (err) => {
      ...
      if (respObj.sync_user && (apiType % 2 !== 0) && respObj.sync_re_code + '' === '0') {
        ...
        if (respObj.plus) {
          for (const j in respObj.plus) {
            userRelationUpdate[j] = respObj.plus[j];
          }
        }
...
const orderLog = function orderLog(apiName, action, actionRe, reqObj, productObj) {
  ...
  if (actionRe.updateOrderLog) {  //如果有updateOrderLog，则进行更新操作，不新建order
    const updateQuery = actionRe.updateOrderLog;
    delete saveObj.updateOrderLog;
    db.c(apiName + '_order').updateOne(updateQuery, { '$set': saveObj });
    return;
  }
