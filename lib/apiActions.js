...
const findKeyByProduct = function findKeyByProduct(respObj, userRelationUpdate, callback) {
  ...
  db.c(productTable).findOne(respObj.keyFromProduct, (e, doc) => {
    if (e) {
      return callback(vlog.ee(e, 'syncAction:findKeyByProduct:产品表查key失败', respObj));
    }
    if (!doc) {
      return callback(vlog.ee(new Error('syncAction:findKeyByProduct:产品表中找不到对应key'), respObj));
    }
    ...
    provinceUtil.findProvinceByPhone(respObj.sync_user, (err, provinceName) => {
      if (err) {
        return callback(vlog.ee(err, 'syncAction:findKeyByProduct:findProvinceByPhone', respObj));
      }
...
const syncAction = function syncAction(apiName, apiType, action, respObj) {
  ...
  syncFindOrder(apiName, respObj.orderQuery, function(err, orderObj) {
    ...
    findKeyByProduct(respObj, userRelationUpdate, (err, re) => {
      if (err) {
