...
const reqCheck = function(reqTxt, reqCheckArr, callback) {
  let reqObj = null;
  try {
    reqObj = JSON.parse(reqTxt);
  } catch (e) {
    // db.c(orderTable,).logToDb( 'state': errorCode.err['reqJson'], 'reqTxt': reqTxt });
    return callback(vlog.ee(e, 'jsonParse'), mkResp(errorCode.err['reqJson'], { 'desc': 'Json格式错误' }));
  }
  apiActions.findProduct(reqObj.productKey, (err, productObj) => {
    if (err) {
      return callback(vlog.ee(err, 'reqCheck:findProduct', reqTxt), mkResp(errorCode.err['findProduct'], { 'desc': '产品未知错误' }));
    }
    if (!productObj || productObj.state < 10) {
      return callback(null, reqObj); //产品不存在或已下线，这里不返回productObj
    }
    if (reqCheckArr.length <= 0) {
      return callback(null, reqObj, productObj);
    }

    const paraCheckArr = [];
    const checkArr = [];
    for (let j = 0; j < reqCheckArr.length; j++) {
      checkArr.push([reqObj[reqCheckArr[j][0]], null, reqCheckArr[j][1]]);
      paraCheckArr.push(reqCheckArr[j][0]);
    }
    const checkRe = cck.checkBatch(checkArr);
    if (checkRe.length > 0) {
      for (let i = 0; i < checkRe.length; i++) {
        checkRe[i] = paraCheckArr[parseInt(checkRe[i])] || checkRe[i];
      }
      return callback(vlog.ee(new Error('reqCheck'), 'reqCheck'), mkResp(errorCode.err['reqErr'], { 'desc': '参数错误', 'errParas': checkRe }));
    }
    ...
    if (sign !== reqObj.sign) {
      return callback(vlog.ee(new Error('reqCheck:signErr'), 'signErr', sign), mkResp(errorCode.err['signErr'], { 'desc': '签名错误' }));
    }
...
const createServer = function createServer(serverName, respFun) {
  const apiServer = http.createServer(function(req, resp) {
    ...
    req.on('end', function() {
      ...
      respFun(req, body, function(err, respObj) {
        if (err) {
          vlog.eo(err, 'api req error', body);
        }
        //!! 无论是否错误,respObj保证有值,必须返回
        respObj = respObj || { 're': 500 };
        httpRe(resp, 200, JSON.stringify(respObj));
      });
    ...
    const respFun = function respFun(req, body, callback) {
      ...
      reqCheck(body, action.reqParas, (err, reqCheckRe, productObj) => {
        ...
        if (!productObj) {
          ...
          return callback(null, mkResp(errorCode.err['productDown'], { 'desc': '产品不存在或已下线' }));
        }
        const riskChecker = action.riskChecker || {
          check(reqObj, productObj, callback) {
            callback(null, { 'code': 0, 'desc': '' });
          }
        };
        riskChecker.check(reqCheckRe, productObj, (err, riskCheckRe) => {
          if (err) {
            return callback(vlog.ee(err, 'riskChecker', body), mkResp(errorCode.err.riskCheck, { 'desc': '风控数据异常' }));
          }
          ...
          action.doAction(reqCheckRe, productObj, (err, actionRe) => {
            if (err) {
              return callback(vlog.ee(err, 'doAction', reqCheckRe)); //异常情况不需要mkResp,外部会处理成500错误
            }
  ...
  sync(apiName, serverConf) {
    const respFun = function respFun(req, body, callback) {
      ...
      syncAction.doAction(req, body, (err, actionRe) => {
        ...
        if (logAction) {
          logAction(apiName, syncAction, actionRe);
        }
      });
    };
    return createServer(serverConf.serverName + '_out', respFun);
  }
};
...
const instance = function instance(apiName, apiType, servers) {
  ...
  for (let i = 0, len = servers.length; i < len; i++) {
    ...
    const serverOne = serverCreator(me.apiName, serverConfOne);
